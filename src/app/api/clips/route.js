...
export const GET = withAuth(async (request) => {
    try {
        ...
        if (user.role === 'studio') {
            ...
            clips = await db.collection('clips')
                .find(query)
                .sort({ createdAt: -1 })
                .toArray();
            ...
            for (let clip of clips) {
                const accessCount = await db.collection('clip_access')
            ...
            const accessRecords = await db.collection('clip_access')
                .find({
            ...
            if (accessRecords.length > 0) {
                ...
                clips = clips.map(clip => {
                    const access = accessRecords.find(record =>
                        record.clipId.toString() === clip._id.toString()
                    );
                    return {
                        ...clip,
        ...
        if (clips.length > 0) {
            ...
            const podcasts = await db.collection('podcasts')
                .find({ _id: { $in: podcastIds } })
                .toArray();

            clips = clips.map(clip => ({
                ...clip,
                podcastInfo: podcasts.find(p => p._id.toString() === clip.podcastId.toString())
            }));
        }
...
// Create new clip (agency only)
export const POST = withAuth(async (request) => {
    try {
        const data = await request.json();
        const user = request.user;

        // Verify agency has access to the podcast
        const { db } = await connectToDatabase();
        ...
        if (!access) {
            return NextResponse.json({
                success: false,
                error: 'No access to this podcast'
            }, { status: 403 });
        }
